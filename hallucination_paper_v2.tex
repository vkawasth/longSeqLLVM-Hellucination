...
% =============================================================================
\section{Peer-Review Corrections: Framework Revision}
\label{sec:peer-review-corrections}
% =============================================================================

The framework as originally stated contains several mathematical errors
identified in peer review. We document and correct them explicitly.

\subsection*{Correction 1: The three invariants are not equivalent projections}

The original claim was:
\begin{quote}
``sheaf failure $=$ Bockstein failure $=$ dynamical orbit growth failure ---
three projections of one geometric object.''
\end{quote}
This is \textbf{false}. These are genuinely different failure modes
living in different mathematical categories:
\begin{center}
\renewcommand{\arraystretch}{1.3}\small
\begin{tabular}{lll}
\toprule
...
\midrule
Sheaf coboundary $\rho_R - \rho_L$ & Locality/consistency & $H^1(K; \mathcal{F})$ \\
...
\end{tabular}
\end{center}
They are not equivalent obstructions. They are a \emph{triangulation system}.
The correct formulation (replacing the co-violation score) is:
\[
  H \;=\; \alpha \nabla C + \beta F + \gamma E
\]
where $C$, $F$, $E$ are \emph{independent} measurements, not equivalent
projections of a shared tensor.
The 0.667 co-violation score reported in earlier drafts is meaningless
because $A$, $B$, $C$ are not independent observables
without proof of conditional independence or a shared latent space.

\subsection*{Correction 2: Random model uniformity is not signal collapse}

The original interpretation was:
\begin{quote}
``Random-init GPT-2 gives uniform signals across all metrics,
therefore signals are degenerate and uninformative.''
\end{quote}
This is \textbf{backwards}. A random model is the maximal-symmetry regime:
$T \approx$ isotropic linear map, no stable eigenstructure, no persistent attractors.
Everything fires because \emph{nothing is constrained}.
This is absence of geometric constraints, not degenerate signal.
Random models are not a zero-structure baseline --- they are a
maximal-symmetry-collapse baseline.
That is why they are bad null models for topology-based hallucination detection.
The correct null model is a trained checkpoint where semantic structure
exists but factual calibration has not yet emerged.

\subsection*{Correction 3: Obstruction classes are not Galois orbits}

The original claim was:
\begin{quote}
``Obstruction classes over $\mathbb{F}_p$ are exactly Galois orbits
under $\mathrm{Gal}(\bar{\mathbb{F}}_p/\mathbb{F}_p)$.''
\end{quote}
This is \textbf{not correct as stated}.
Obstruction classes live in cohomology groups / derived functors / extension classes.
...
Galois-equivariant category, which requires additional structure
we do not establish.
The corrected statement: the three primes $p \in \{2,5,7\}$ are
\emph{different localizations} of the same descent obstruction functor,
not semantic labels encoding different hallucination types.

\subsection*{Correction 4: Scalar extension does not preserve all structure}

The original claim was:
\begin{quote}
``Working over $\mathbb{F}_p$ vs $\bar{\mathbb{F}}_p$ loses no information ---
only Galois orbits.''
\end{quote}
This is \textbf{overstated}. Scalar extension $\mathbb{F}_p \to \bar{\mathbb{F}}_p$
loses Frobenius-semilinear structure and descent data.
The correct statement: scalar extension forgets descent data, and what
remains are Galois orbits of geometric constituents.
Reconstructing the original object requires that descent data
(the Galois action encoding the field of definition).
We use the Galois orbit structure as a \emph{proxy}, not as a literal model.

\subsection*{Correction 5: Supersingularity is not equivalent to hallucination-freedom}

The original claim was:
\begin{quote}
``A trajectory is hallucination-free at prime $p$ if and only if it is
supersingular (not parabolically induced from any proper Levi subgroup).''
\end{quote}
This is \textbf{incorrect}. Hallucination is semantic misalignment with
external ground truth. Supersingularity/irreducibility is structural
non-decomposability. These are \emph{orthogonal} properties.
A model can be globally coherent (even irreducible as a representation)
while producing unsupported factual content.
The correct reformulation:
\begin{quote}
A trajectory is hallucination-free if it satisfies \emph{descent consistency}
across all context filtrations: its global generation is uniquely determined
by compatible local conditional distributions.
\end{quote}
The Levi/parabolic language is retained but reinterpreted:
\begin{itemize}
  \item Levi subgroup $= $ context factorization system (NOT supersingularity)
  \item Parabolic induction $= $ compositional decoding from shorter context
  \item Obstruction $= $ failure of descent, not failure of irreducibility
\end{itemize}

\subsection*{The one correct unified statement}

After all corrections, one statement survives intact and is
mathematically defensible:
\begin{quote}
\textbf{Hallucination $\sim$ failure of descent from local data.}
A trajectory is hallucination-free if and only if it satisfies
descent consistency across all context filtrations, i.e., its global
generation is uniquely determined by compatible local conditional
distributions.
\end{quote}
This supports the framework:
$\text{sheaves} = \text{context windows}$,
$\text{sections} = \text{trajectories}$,
$\text{obstruction} = \text{hallucination}$,
$\text{induction} = \text{compositional decoding}$.
